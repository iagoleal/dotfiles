#!/bin/bash

# Theme settings
herbstclient attr theme.tiling.reset 1
herbstclient attr theme.floating.reset 1
herbstclient set frame_border_active_color '#ad3514'
herbstclient set frame_border_normal_color '#101010'
herbstclient set frame_bg_normal_color     '#565656'
herbstclient set frame_bg_active_color     '#345F0C'
herbstclient set frame_border_width 1
herbstclient set always_show_frame 0
herbstclient set frame_bg_transparent 1
herbstclient set frame_transparent_width 0
herbstclient set frame_gap 5

herbstclient attr theme.active.color        #afafaf'
herbstclient attr theme.normal.color       '#5f5f5f'
herbstclient attr theme.urgent.color orange
herbstclient attr theme.inner_width 1
herbstclient attr theme.inner_color #afafaf"
herbstclient attr theme.border_width 2
herbstclient attr theme.floating.border_width 1
herbstclient attr theme.floating.outer_width 0
herbstclient attr theme.floating.outer_color black
herbstclient attr theme.active.inner_color '#afafaf'
herbstclient attr theme.active.outer_color '#afafaf'
herbstclient attr theme.background_color   '#141414'

herbstclient set window_gap 5
herbstclient set frame_padding 0
herbstclient set smart_window_surroundings 1
herbstclient set smart_frame_surroundings 1
herbstclient set mouse_recenter_gap 0
